...
function getFixedGuide(guideMode, canvasW, canvasH) {
  ...
  const rw = canvasW * wR, rh = canvasH * hR;
  return { rx: (canvasW - rw) / 2, ry: canvasH * yOff, rw, rh };
}

/* ─── FACE YAW from landmarks (video coords, unmirrored) ─── */
function getFaceYaw(faceData) {
...
function validatePosition(faceData, angle, canvasW, canvasH, mirrored) {
  ...
  const sx = canvasW / faceData.videoW, sy = canvasH / faceData.videoH;
...
/* ─── GUIDE OVERLAY ─── */
function GuideOverlay({ faceData, angle, canvasW, canvasH, detectFace, mirrored, validation }) {
  const isReady = validation.status === "ready";
  const isWrong = validation.status === "wrong_dir";
  const stroke = isReady ? "#16a34a" : isWrong ? "#dc2626" : "#d97706";
  const fill = isReady ? "rgba(22,163,106,0.04)" : isWrong ? "rgba(220,38,38,0.06)" : "rgba(217,119,6,0.02)";
  const { rx, ry, rw, rh } = getFixedGuide(angle.guideMode, canvasW, canvasH);
  ...
  if (lm?.length > 0) {
    const px = (i) => { if (!lm[i]) return 0; const x = lm[i].x * sx; return mirrored ? canvasW - x : x; };
    const py = (i) => lm[i] ? lm[i].y * sy : 0;
    const path = (ids) => ids.filter(i => lm[i]).map((i, j) => `${j === 0 ? "M" : "L"}${px(i).toFixed(1)},${py(i).toFixed(1)}`).join(" ");
    jawPath = path(JAWLINE); lipsPath = path(LIPS_OUTER); nosePath = path(NOSE);
  }

  return (
    <svg style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none" }}
      viewBox={`0 0 ${canvasW} ${canvasH}`}>
      <defs><mask id="gm"><rect width={canvasW} height={canvasH} fill="white" /><rect x={rx} y={ry} width={rw} height={rh} rx="10" fill="black" /></mask></defs>
      <rect width={canvasW} height={canvasH} fill="rgba(0,0,0,0.5)" mask="url(#gm)" />
      <rect x={rx} y={ry} width={rw} height={rh} rx="10" fill={fill} stroke={stroke} strokeWidth="2.5" strokeDasharray={isReady ? "none" : "8 5"} />
      ...
      {jawPath && <path d={jawPath} fill="none" stroke={isReady ? "#22c55e" : isWrong ? "#ef4444" : "#eab308"} strokeWidth="2" opacity="0.75" strokeLinejoin="round" />}
      {lipsPath && <path d={lipsPath} fill="none" stroke={isReady ? "#22c55e" : isWrong ? "#ef4444" : "#eab308"} strokeWidth="1.5" opacity="0.55" strokeLinejoin="round" />}
      {nosePath && <path d={nosePath} fill="none" stroke={isReady ? "#22c55e" : isWrong ? "#ef4444" : "#eab308"} strokeWidth="1.5" opacity="0.45" strokeLinejoin="round" />}
      {lm?.[1] && (() => { const nx = mirrored ? canvasW - lm[1].x*sx : lm[1].x*sx; return <circle cx={nx} cy={lm[1].y*sy} r="3" fill={isReady?"#22c55e":"#eab308"} opacity="0.7" />; })()}

      {/* BIG ANGLE LABEL at bottom of camera */}
      <rect x={rx} y={ry + rh - 40} width={rw} height={40} rx="0 0 10 10" fill="rgba(0,0,0,0.6)" />
      <text x={rx + rw/2} y={ry + rh - 14} textAnchor="middle" fontSize="20" fontWeight="800" fill="#fff" fontFamily="system-ui">
        {angle.bigLabel}
      </text>
    </svg>
  );
...
function CameraView({ onCapture, angle, onBack, angleIndex, totalAngles }) {
  ...
  const mediapipeRef = useRef(null); const detectingRef = useRef(false);

  const startCamera = useCallback(async (facing) => {
    try {
      if (streamRef.current) streamRef.current.getTracks().forEach(t => t.stop());
      setCameraReady(false); setError(null);
      const stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: facing, width: { ideal: 1280 }, height: { ideal: 1600 } }, audio: false });
      streamRef.current = stream;
      if (videoRef.current) { videoRef.current.srcObject = stream; videoRef.current.onloadedmetadata = () => { const vw = videoRef.current.videoWidth, vh = videoRef.current.videoHeight; const dw = Math.min(420, window.innerWidth - 24); setDims({ w: dw, h: Math.round(dw * (vh / vw)) }); setCameraReady(true); }; }
    } catch { setError("Camera access denied. Allow permissions and reload."); }
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const r = await vision.FilesetResolver.forVisionTasks("https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.14/wasm");
        if (cancelled) return;
        const l = await vision.FaceLandmarker.createFromOptions(r, { baseOptions: { modelAssetPath: "https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/1/face_landmarker.task", delegate: "GPU" }, runningMode: "VIDEO", numFaces: 1 });
        if (cancelled) return;
        mediapipeRef.current = { type: "mediapipe", detector: l }; setDetectorType("mediapipe"); return;
      } catch (e) { console.log("MediaPipe:", e.message); }
      if ("FaceDetector" in window) { try { mediapipeRef.current = { type: "native", detector: new window.FaceDetector({ fastMode: true, maxDetectedFaces: 1 }) }; if (!cancelled) setDetectorType("native"); return; } catch {} }
  ...
  useEffect(() => {
    ...
    const detect = async () => {
      ...
      detectingRef.current = true; const v = videoRef.current;
      try {
        if (mediapipeRef.current?.type === "mediapipe") {
          const now = performance.now(); if (now - lastT < 100) { detectingRef.current = false; if (running) requestAnimationFrame(detect); return; } lastT = now;
          const res = mediapipeRef.current.detector.detectForVideo(v, now);
          if (res.faceLandmarks?.length > 0) {
            const lm = res.faceLandmarks[0]; const vw = v.videoWidth, vh = v.videoHeight;
            let mnX=Infinity,mnY=Infinity,mxX=-Infinity,mxY=-Infinity;
            const pts = lm.map(p => { const px=p.x*vw,py=p.y*vh; if(px<mnX)mnX=px;if(py<mnY)mnY=py;if(px>mxX)mxX=px;if(py>mxY)mxY=py; return{x:px,y:py}; });
            setFaceData({ detected:true, bbox:{x:mnX,y:mnY,width:mxX-mnX,height:mxY-mnY}, allLandmarks:pts, videoW:vw, videoH:vh });
          } else { setFaceData(p => p ? {...p,detected:false,allLandmarks:null} : null); }
        } else if (mediapipeRef.current?.type === "native") {
          const faces = await mediapipeRef.current.detector.detect(v);
          if (faces.length>0) { const bb=faces[0].boundingBox; setFaceData({detected:true,bbox:{x:bb.x,y:bb.y,width:bb.width,height:bb.height},allLandmarks:null,videoW:v.videoWidth,videoH:v.videoHeight}); }
          else { setFaceData(p => p ? {...p,detected:false,allLandmarks:null} : null); }
        }
      } catch {}
      detectingRef.current = false; if (running) requestAnimationFrame(detect);
    };
    detect(); return () => { running = false; };
  }, [cameraReady, detectorType, angle.detectFace]);

  useEffect(() => { if (!angle.detectFace) setFaceData(null); }, [angle.detectFace]);

  const isMirrored = facingMode === "user";
  const validation = validatePosition(faceData, angle, dims.w, dims.h, isMirrored);

  const capturePhoto = useCallback(() => {
    const video = videoRef.current; if (!video) return;
    const vw = video.videoWidth, vh = video.videoHeight;
    const g = getFixedGuide(angle.guideMode, vw, vh);
    let gx=Math.max(0,Math.round(g.rx)), gy=Math.max(0,Math.round(g.ry));
    let gw=Math.min(Math.round(g.rw),vw-gx), gh=Math.min(Math.round(g.rh),vh-gy);
    const c = canvasRef.current; c.width=gw; c.height=gh;
    const ctx = c.getContext("2d");
    if (facingMode==="user") { ctx.drawImage(video,vw-gx-gw,gy,gw,gh,0,0,gw,gh); }
    else { ctx.drawImage(video,gx,gy,gw,gh,0,0,gw,gh); }

    // Eye privacy bar for front face
    if (angle.guideMode === "front" && faceData?.allLandmarks?.length > 0) {
      const lm = faceData.allLandmarks;
      const eyeIdx = [33,133,159,145,160,144,153,158,263,362,386,374,385,373,380,387];
      let eMinY=Infinity, eMaxY=-Infinity;
      eyeIdx.forEach(i => { if(lm[i]) { const ly=lm[i].y-gy; if(ly<eMinY)eMinY=ly; if(ly>eMaxY)eMaxY=ly; } });
      if (eMinY < eMaxY) {
        const pad = (eMaxY-eMinY)*0.6;
        ctx.fillStyle = "#000"; ctx.fillRect(0, Math.max(0,eMinY-pad), gw, (eMaxY-eMinY)+pad*2);
      }
    }
  ...
  const isReady = validation.status === "ready";
  const isWrong = validation.status === "wrong_dir";
  const badgeColor = isReady ? "rgba(22,163,106,0.9)" : isWrong ? "rgba(220,38,38,0.9)" : faceData?.detected ? "rgba(234,179,8,0.9)" : "rgba(100,116,139,0.8)";

  if (error) return (<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40,textAlign:"center",gap:16,minHeight:300}}><AlertCircle size={40} style={{color:"#dc2626"}} /><p style={{color:"#64748b",fontSize:14,maxWidth:280}}>{error}</p><button onClick={()=>startCamera(facingMode)} style={btnPrimary}>Retry</button></div>);
    ...
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",width:"100%" }}>
      <div style={{ background:"#fff",borderBottom:"1px solid #e2e8f0",padding:"10px 16px",width:"100%",display:"flex",alignItems:"center",gap:10 }}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:12,color:"#64748b",fontWeight:500}}>{angle.instruction}</div>
        </div>
      ...
      <div style={{display:"flex",gap:3,padding:"8px 16px",width:"100%",background:"#fff"}}>
        {Array.from({length:totalAngles}).map((_,i) => (<div key={i} style={{flex:1,height:3,borderRadius:2,background:i<angleIndex?"#16a34a":i===angleIndex?"#2563eb":"#e2e8f0"}} />))}
      </div>
      <div style={{position:"relative",width:dims.w,height:dims.h,background:"#000",borderRadius:12,overflow:"hidden",margin:"0 12px"}}>
        ...
        {cameraReady && <GuideOverlay faceData={faceData} angle={angle} canvasW={dims.w} canvasH={dims.h} detectFace={angle.detectFace} mirrored={isMirrored} validation={validation} />}
        ...
        {cameraReady && angle.detectFace && (
          <div style={{position:"absolute",top:12,left:12,right:12,display:"flex",alignItems:"center",gap:6,background:badgeColor,borderRadius:10,padding:"6px 12px"}}>
            <CircleDot size={14} color="#fff" />
            <span style={{color:"#fff",fontSize:13,fontWeight:700,flex:1}}>{validation.msg}</span>
          </div>
        )}
...
function PhotoReview({ photo, angle, onRetake, onAccept }) {
  return (<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16,padding:"24px 16px"}}>
    <div style={{fontSize:18,fontWeight:800,color:"#0f172a"}}>{angle.label}</div>
...
async function uploadToDrive(url, pid, angle, img) {
  return (await fetch(url, { method:"POST", body:JSON.stringify({patientId:pid,angle,image:img.split(",")[1]}) })).json();
}
...
export default function App() {
  ...
  const startNew = () => { const id=generatePatientId(nextSerial); const ns=nextSerial+1; setNextSerial(ns); setPatientId(id); const u={...patients,[id]:{photos:{},createdAt:new Date().toISOString()}}; setPatients(u); persist(u,ns); setCurrentAngle(0); setCapturedPhoto(null); setScreen("capture"); };
  const resume = (pid) => { setPatientId(pid); const n=ANGLES.findIndex(a=>!patients[pid]?.photos[a.id]); setCurrentAngle(n>=0?n:0); setCapturedPhoto(null); setScreen("capture"); };
  ...
  const dlAll = async(pid) => { for(const[a,d]of Object.entries(patients[pid]?.photos||{})){const l=document.createElement("a");l.href=d;l.download=`${pid}_${a}.jpg`;document.body.appendChild(l);l.click();document.body.removeChild(l);await new Promise(r=>setTimeout(r,350));} };
  const uploadAll = async(pid) => { if(!driveUrl){setUploadMsg("Set Apps Script URL in Settings.");return;} setUploading(true);setUploadMsg(""); try{for(const[a,d]of Object.entries(patients[pid]?.photos||{})){await uploadToDrive(driveUrl,pid,a,d);}setUploadMsg(`Uploaded ${Object.keys(patients[pid]?.photos||{}).length} photos`);}catch{setUploadMsg("Upload failed.");} setUploading(false); };
  const delPat = (pid) => { const u={...patients};delete u[pid];setPatients(u);persist(u);if(viewingPatient===pid){setScreen("home");setViewingPatient(null);} };
  const cc = (pid) => Object.keys(patients[pid]?.photos||{}).length;
  const pl = Object.entries(patients).sort((a,b)=>(b[1].createdAt||"").localeCompare(a[1].createdAt||""));
  ...
  const modal = showSettings && (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setShowSettings(false)}>
      <div style={{background:"#fff",borderRadius:16,padding:24,maxWidth:400,width:"100%",boxShadow:"0 20px 60px rgba(0,0,0,0.12)"}} onClick={e=>e.stopPropagation()}>
        ...
        <label style={labelSt}>Apps Script URL</label>
        <input value={driveUrl} onChange={e=>setDriveUrl(e.target.value)} placeholder="https://script.google.com/macros/s/.../exec" style={inputSt} />
  ...
  if(screen==="home") return (
    <div style={ctr}>{modal}
      ...
      <div style={card}><button onClick={startNew} style={{...btnPrimary,width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"14px 20px",fontSize:15}}><Camera size={18} /> New Patient Session</button><p style={{fontSize:11,color:"#94a3b8",marginTop:8,textAlign:"center",fontFamily:"monospace"}}>Next: {generatePatientId(nextSerial)}</p></div>
      ...
      {pl.length>0 && <div style={card}><div style={{fontSize:12,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:10}}>Patients ({pl.length})</div>
        {pl.map(([pid])=>{const c=cc(pid);const done=c===ANGLES.length;return(<div key={pid} style={{background:"#f8fafc",borderRadius:10,padding:"10px 12px",display:"flex",alignItems:"center",gap:10,border:done?"1px solid #bbf7d0":"1px solid #f1f5f9",marginBottom:6}}><div style={{width:36,height:36,borderRadius:10,background:done?"#dcfce7":"#f1f5f9",display:"flex",alignItems:"center",justifyContent:"center"}}>{done?<Check size={16} color="#16a34a"/>:<User size={16} color="#94a3b8"/>}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,fontFamily:"monospace"}}>{pid}</div><div style={{fontSize:11,color:"#94a3b8"}}>{c}/{ANGLES.length}</div></div><div style={{display:"flex",gap:2}}><button onClick={()=>{setViewingPatient(pid);setScreen("gallery");}} style={iconBtn}><Eye size={15}/></button><button onClick={()=>resume(pid)} style={iconBtn}><Camera size={15}/></button><button onClick={()=>dlAll(pid)} style={iconBtn}><Download size={15}/></button>{driveUrl&&<button onClick={()=>uploadAll(pid)} style={iconBtn}><Upload size={15}/></button>}<button onClick={()=>delPat(pid)} style={{...iconBtn,color:"#ef4444"}}><Trash2 size={15}/></button></div></div>);})}
        {uploadMsg&&<p style={{fontSize:12,color:uploadMsg.includes("fail")?"#dc2626":"#16a34a",marginTop:8}}>{uploadMsg}</p>}
      </div>}
      <div style={{padding:"12px 20px 24px"}}><div style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:10,padding:12,display:"flex",gap:8}}><Shield size={16} style={{color:"#16a34a",flexShrink:0,marginTop:1}} /><p style={{fontSize:11,color:"#15803d",lineHeight:1.6,margin:0}}>Photos cropped symmetrically and stored locally. MediaPipe cached after first load.</p></div></div>
    </div>
  );

  if(screen==="capture") return (<div style={ctr}><CameraView onCapture={handleCapture} angle={ANGLES[currentAngle]} angleIndex={currentAngle} totalAngles={ANGLES.length} onBack={()=>setScreen("home")} /><div style={{display:"flex",justifyContent:"space-between",padding:"4px 16px 16px",width:"100%"}}><button disabled={currentAngle===0} onClick={()=>{setCurrentAngle(currentAngle-1);setCapturedPhoto(null);}} style={{...btnSecondary,opacity:currentAngle===0?0.35:1,fontSize:13,display:"flex",alignItems:"center",gap:4}}><ChevronLeft size={14} /> Prev</button><button onClick={()=>{if(currentAngle<ANGLES.length-1){setCurrentAngle(currentAngle+1);setCapturedPhoto(null);}else{setViewingPatient(patientId);setScreen("gallery");}}} style={{...btnSecondary,fontSize:13,display:"flex",alignItems:"center",gap:4}}>Skip <ChevronRight size={14} /></button></div></div>);

  if(screen==="review") return (<div style={ctr}><PhotoReview photo={capturedPhoto} angle={ANGLES[currentAngle]} onRetake={()=>{setCapturedPhoto(null);setScreen("capture");}} onAccept={handleAccept} /></div>);

  if(screen==="gallery"){const pid=viewingPatient;const photos=patients[pid]?.photos||{};return(<div style={ctr}><div style={{padding:16,width:"100%"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><button onClick={()=>setScreen("home")} style={iconBtn}><ArrowLeft size={20}/></button><div style={{flex:1}}><div style={{fontSize:17,fontWeight:800}}>{pid}</div><div style={{fontSize:12,color:"#64748b"}}>{Object.keys(photos).length}/{ANGLES.length}</div></div><button onClick={()=>dlAll(pid)} style={{...btnPrimary,fontSize:12,padding:"8px 14px",display:"flex",alignItems:"center",gap:5}}><Download size={14}/> Export</button>{driveUrl&&<button onClick={()=>uploadAll(pid)} disabled={uploading} style={{...btnSecondary,fontSize:12,padding:"8px 14px",display:"flex",alignItems:"center",gap:5}}>{uploading?<Loader2 size={14} style={{animation:"spin 1s linear infinite"}}/>:<Upload size={14}/>} Drive</button>}</div>{uploadMsg&&<p style={{fontSize:12,color:uploadMsg.includes("fail")?"#dc2626":"#16a34a",marginBottom:10}}>{uploadMsg}</p>}<div style={{display:"grid",gridTemplateColumns:"repeat(2, 1fr)",gap:10}}>{ANGLES.map(a=>(<div key={a.id} style={{background:"#fff",borderRadius:12,overflow:"hidden",border:photos[a.id]?"2px solid #bbf7d0":"2px dashed #e2e8f0"}}>{photos[a.id]?<img src={photos[a.id]} alt={a.label} style={{width:"100%",aspectRatio:"3/4",objectFit:"cover",display:"block"}}/>:<div style={{width:"100%",aspectRatio:"3/4",display:"flex",alignItems:"center",justifyContent:"center",background:"#f8fafc"}}><Camera size={28} style={{color:"#e2e8f0"}}/></div>}<div style={{padding:"8px 10px",display:"flex",alignItems:"center",justifyContent:"space-between"}}><span style={{color:"#64748b",fontSize:11,fontWeight:600}}>{a.label}</span>{photos[a.id]?<Check size={14} style={{color:"#16a34a"}}/>:<button onClick={()=>resume(pid)} style={{background:"none",border:"none",color:"#2563eb",cursor:"pointer",fontSize:11,fontWeight:700}}>Capture</button>}</div></div>))}</div>{Object.keys(photos).length<ANGLES.length&&<button onClick={()=>resume(pid)} style={{...btnPrimary,background:"#16a34a",width:"100%",marginTop:16,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}><Camera size={16}/> Continue</button>}</div></div>);}
  return null;
}

const ctr = {minHeight:"100vh",background:"#ffffff",fontFamily:"'DM Sans', system-ui, sans-serif",display:"flex",flexDirection:"column",alignItems:"center",maxWidth:480,margin:"0 auto"};
...
const labelSt = {display:"block",fontSize:12,fontWeight:600,color:"#475569",marginBottom:4};
const inputSt = {width:"100%",background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:8,padding:"10px 12px",fontSize:14,color:"#0f172a",boxSizing:"border-box"};
